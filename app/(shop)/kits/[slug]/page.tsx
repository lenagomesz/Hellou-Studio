...
import { buildProductKits, KIT_DEFINITIONS } from '@/lib/product-kits';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const kit = KIT_DEFINITIONS.find(item => item.slug === slug);
...
export default async function KitPage({ params }: Props) {
  const { slug } = await params;
  if (!KIT_DEFINITIONS.some(item => item.slug === slug)) notFound();
  const [products, settings] = await Promise.all([getKitCatalog(), getStoreSettings()]);
  const kit = buildProductKits(products).find(item => item.slug === slug);
