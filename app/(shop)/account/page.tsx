...
export default async function AccountPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Welcome Header */}
      <section className="rounded-2xl border border-gray-100 bg-gradient-to-br from-white via-white to-pink-50/40 p-6 shadow-sm sm:p-8">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
        <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
          ...
          <p className="mt-0.5 text-xs text-gray-500">{stats.total === 1 ? 'Pedido' : 'Pedidos'}</p>
        </div>
        <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
          ...
          <p className="mt-3 text-2xl font-bold text-gray-900">{stats.active}</p>
          <p className="mt-0.5 text-xs text-gray-500">Em andamento</p>
        </div>
        <div className="col-span-2 sm:col-span-1 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
          ...
          <p className="mt-0.5 text-xs text-gray-500">Entregues</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Link
          href="/products"
          className="group flex items-center gap-3 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm transition hover:border-pink-200 hover:shadow-md"
        >
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-pink-50 to-orange-50 transition group-hover:from-pink-100 group-hover:to-orange-100">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="h-5 w-5 text-pink-600">
              <path strokeLinecap="round" strokeLinejoin="round" d="M13.5 21v-7.5a.75.75 0 0 1 .75-.75h3a.75.75 0 0 1 .75.75V21m-4.5 0H2.36m11.14 0H18m0 0h3.64m-1.39 0V9.349M3.75 21V9.349m0 0a3.001 3.001 0 0 0 3.75-.615A2.993 2.993 0 0 0 9.75 9.75c.896 0 1.7-.393 2.25-1.016a2.993 2.993 0 0 0 2.25 1.016c.896 0 1.7-.393 2.25-1.015a3.001 3.001 0 0 0 3.75.614m-16.5 0a3.004 3.004 0 0 1-.621-4.72l1.189-1.19A1.5 1.5 0 0 1 5.378 3h13.243a1.5 1.5 0 0 1 1.06.44l1.19 1.189a3 3 0 0 1-.621 4.72M6.75 18h3.75a.75.75 0 0 0 .75-.75V13.5a.75.75 0 0 0-.75-.75H6.75a.75.75 0 0 0-.75.75v3.75c0 .414.336.75.75.75Z" />
            </svg>
          </div>
          <div>
            <p className="text-sm font-semibold text-gray-900">Catálogo</p>
            <p className="text-xs text-gray-500">Ver produtos</p>
          </div>
        </Link>
        <Link
          href="/request-print"
        ...
        >
          ...
          <div>
            <p className="text-sm font-semibold text-gray-900">Encomenda</p>
            <p className="text-xs text-gray-500">Peça personalizada</p>
          </div>
        </Link>
      </div>
      ...
      <section className="rounded-2xl border border-gray-100 bg-white shadow-sm overflow-hidden">
        <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4 sm:px-6">
          ...
          <Link href="/account/orders" className="text-xs font-medium text-pink-600 hover:text-pink-700 transition">
            Ver todos →
          </Link>
        </div>
        ...
        {recentOrders.length === 0 ? (
          <div className="p-10 text-center">
            ...
            <p className="mt-1 text-xs text-gray-500">Explore nosso catálogo e encontre peças incríveis!</p>
            <Link
              href="/products"
            ...
            {recentOrders.map((order) => (
              <li key={order.id}>
                <Link
                  ...
                  className="group flex items-center justify-between gap-3 px-5 py-4 transition hover:bg-gray-50/80 sm:px-6"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gray-50 transition group-hover:bg-gray-100">
                  ...
                  <div className="flex flex-wrap items-center justify-end gap-2">
                    ...
                    <span className="text-sm font-semibold text-gray-900 whitespace-nowrap">{formatPrice(order.total)}</span>
