...
export async function POST(request: Request) {
  ...
  const invalidCartItem = cartItems.find((item) =>
    ...
    || item.quantity < 1
    || item.quantity > (item.product?.is_wholesale ? 1000 : 50)
    || (item.product?.is_wholesale && item.quantity !== 1 && item.quantity < Math.max(2, item.product.minimum_order_quantity ?? 2))
