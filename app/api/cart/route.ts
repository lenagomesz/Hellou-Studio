...
type RawCartRow = CartItem & {
  ...
  > | null;
  option:
    | Pick<ProductOption, 'id' | 'product_id' | 'name' | 'price_modifier' | 'stock' | 'color' | 'color_name' | 'image_url'>
    | null;
};
  ...
  return {
    ...
    option: row.option
      ? {
          ...
          color_name: row.option.color_name,
...
export async function GET() {
  ...
  const { data, error } = await admin
    .from('cart_items')
    .select(
      'id, user_id, product_id, product_option_id, quantity, customization_text, created_at, product:products(id, name, base_price, sale_price, image_url, category, type, active, fulfillment_mode, is_wholesale, minimum_order_quantity), option:product_options(id, product_id, name, price_modifier, stock, color, color_name, image_url)',
    )
