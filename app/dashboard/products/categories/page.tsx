...
export default function ProductCategoriesPage() {
  const [categories, setCategories] = useState<Array<ProductCategory & { product_count?: number }>>([]);
  const [name, setName] = useState('');
  const [tags, setTags] = useState<Array<ProductTag & { product_count?: number }>>([]);
  ...
  async function loadCategories() {
    ...
    const categoryData = await categoryResponse.json().catch(() => ({})) as { categories?: Array<ProductCategory & { product_count?: number }>; error?: string };
    const tagData = await tagResponse.json().catch(() => ({})) as { tags?: Array<ProductTag & { product_count?: number }>; error?: string };
...
function CategoryRow({ category, onSave, onDelete }: { category: ProductCategory & { product_count?: number }; onSave: (category: ProductCategory, update: Partial<ProductCategory>) => Promise<void>; onDelete: (category: ProductCategory) => void | Promise<void> }) {
  ...
  return (
    <div className="grid gap-3 px-5 py-4 sm:grid-cols-[1fr_auto_auto] sm:items-center">
      <div><div className="flex items-center gap-2"><span className="h-4 w-4 shrink-0 rounded-full bg-pink-500" /><input value={name} onChange={(event) => setName(event.target.value)} className="w-full max-w-md rounded-lg border border-transparent bg-gray-50 px-3 py-2 text-sm font-medium text-gray-900 focus:border-pink-300 focus:bg-white focus:outline-none dark:bg-gray-800 dark:text-white" /></div><p className="mt-1 px-6 text-xs text-gray-400">/{category.slug}{category.is_system ? ' · categoria padrão' : ''} · {category.product_count ?? 0} produtos</p></div>
...
function TagRow({ tag, onSave, onDelete }: { tag: ProductTag & { product_count?: number }; onSave: (tag: ProductTag, update: Pick<ProductTag, 'name' | 'color'>) => Promise<void>; onDelete: (tag: ProductTag) => void | Promise<void> }) {
  ...
  return (
    <div className="grid gap-3 px-5 py-4 sm:grid-cols-[1fr_145px_auto] sm:items-center">
      <div><div className="flex items-center gap-2"><span className="rounded-full px-2.5 py-1 text-[10px] font-bold text-white" style={{ backgroundColor: color }}>{name || 'Tag'}</span><input value={name} maxLength={40} onChange={(event) => setName(event.target.value)} className="w-full max-w-md rounded-lg border border-transparent bg-gray-50 px-3 py-2 text-sm font-medium text-gray-900 focus:border-violet-300 focus:bg-white focus:outline-none dark:bg-gray-800 dark:text-white" /></div><p className="mt-1 text-xs text-gray-400">{tag.product_count ?? 0} produtos vinculados</p></div>
