  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-bold text-slate-950">Encomendas registradas</h2><p className="text-sm text-slate-500">{orders.length} no total · {pendingPayments} com pagamento pendente</p></div><div className="flex w-full flex-col gap-2 sm:min-w-[280px] sm:flex-1 sm:flex-row sm:justify-end"><div className="relative w-full sm:max-w-sm sm:flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar cliente ou encomenda" className="w-full rounded-xl border border-slate-200 py-2.5 pl-9 pr-3 text-sm" /></div><select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="min-h-11 w-full rounded-xl border border-slate-200 px-3 text-sm sm:w-auto"><option value="">Todos</option>{MANUAL_ORDER_STATUSES.map((value) => <option key={value} value={value}>{MANUAL_ORDER_STATUS_LABELS[value]}</option>)}</select></div></div>
