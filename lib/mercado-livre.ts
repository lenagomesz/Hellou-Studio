...
export function getMercadoLivreConfig(requestUrl?: string) {
  ...
  if (!appId || !clientSecret || !redirectUri) {
    throw new Error('Configure MERCADO_LIVRE_APP_ID, MERCADO_LIVRE_CLIENT_SECRET e MERCADO_LIVRE_REDIRECT_URI.');
  }
...
async function requestToken(body: URLSearchParams): Promise<MercadoLivreTokenResponse> {
  ...
  const data = await response.json() as MercadoLivreTokenResponse & { message?: string; error_description?: string };
  if (!response.ok || !data.access_token || !data.refresh_token) {
    throw new Error(data.error_description ?? data.message ?? 'O Mercado Livre recusou a autorização.');
  }
  return data;
}
...
export async function getMercadoLivreConnection(adminUserId: string) {
  const { data, error } = await getSupabaseAdmin()
    ...
    .maybeSingle();
  if (error) throw new Error(`Não foi possível consultar a conexão: ${error.message}`);
...
export async function getValidMercadoLivreToken(adminUserId: string, requestUrl?: string) {
  ...
  if (!connection) throw new Error('Conecte sua conta do Mercado Livre antes de sincronizar.');
  ...
  const token = await requestToken(new URLSearchParams({
    ...
    client_id: config.appId,
    client_secret: config.clientSecret,
    refresh_token: decryptMercadoLivreToken(connection.refresh_token_encrypted),
  }));
