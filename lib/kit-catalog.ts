...
export const getKitCatalog = cache(async (): Promise<KitProduct[]> => {
  ...
  try {
    const { data, error } = await withTimeout(getSupabaseAdmin().from('products')
      .select('id,name,description,category,type,base_price,sale_price,image_url,image_url_2,images,active,fulfillment_mode,is_wholesale,is_customizable,customization_question,customization_help_text,customization_placeholder,customization_sections,image_alt_texts,product_options(id,product_id,name,price_modifier,stock,color,color_name,image_url,sort_order,active)')
