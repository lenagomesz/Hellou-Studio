      ...
      <div className="relative w-full px-5 py-10 sm:px-8 lg:px-12 lg:py-14 xl:px-16">
        <div className="grid grid-cols-2 gap-x-7 gap-y-10 md:grid-cols-4 lg:gap-12">
          <div className="col-span-2 md:col-span-1">
            <Link href="/" className="inline-flex text-2xl font-bold tracking-tight" aria-label={`${settings.identity.name} — página inicial`}>
              {settings.identity.logoUrl ? <img src={settings.identity.logoUrl} alt={settings.identity.name} className="h-10 max-w-52 object-contain object-left" /> : <span className="store-gradient-text">{settings.identity.shortName}</span>}
            </Link>
            ...
            <div className="mt-4 flex gap-2">
              {settings.contact.instagram && <a href={settings.contact.instagram} target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="flex h-9 w-9 items-center justify-center rounded-full border border-gray-100 bg-gray-50 text-gray-400 transition hover:-translate-y-0.5 hover:border-pink-200 hover:bg-pink-50 hover:text-pink-500 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-500 dark:hover:border-pink-900 dark:hover:bg-pink-950/30 dark:hover:text-pink-400">
              ...
              {settings.contact.tiktok && <a href={settings.contact.tiktok} target="_blank" rel="noopener noreferrer" aria-label="TikTok" className="flex h-9 w-9 items-center justify-center rounded-full border border-gray-100 bg-gray-50 text-gray-400 transition hover:-translate-y-0.5 hover:border-pink-200 hover:bg-pink-50 hover:text-pink-500 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-500 dark:hover:border-pink-900 dark:hover:bg-pink-950/30 dark:hover:text-pink-400">
      ...
      <div className="relative w-full border-t border-pink-100/70 bg-pink-50/45 dark:border-gray-800 dark:bg-gray-900/60">
        <div className="flex w-full flex-col gap-4 px-5 py-5 sm:px-8 lg:flex-row lg:items-center lg:justify-between lg:px-12 xl:px-16">
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs text-gray-400 dark:text-gray-500">
            ...
            <span>© {new Date().getFullYear()} {settings.identity.shortName}. Todos os direitos reservados.</span>
          </div>
        </div>
      </div>
    </footer>
