...
describe('Melhor Envio shipping quotes', () => {
  ...
  it('ignores unavailable services and keeps Jadlog quotes', () => {
    expect(parseMelhorEnvioQuotes([
  ...
  it('only offers Jadlog, Correios PAC and Correios SEDEX', () => {
    const options = parseMelhorEnvioQuotes([
      ...
      { id: 3, name: 'SEDEX', custom_price: '32.00', custom_delivery_time: 3, company: { name: 'Correios' } },
      { id: 4, name: 'Mini Envios', custom_price: '17.00', custom_delivery_time: 8, company: { name: 'Loggi' } },
      { id: 5, name: '.Package', custom_price: '22.00', custom_delivery_time: 5, company: { name: 'Jadlog' } },
    ]);
    expect(options.map((option) => option.id)).toEqual(['melhor-envio-1', 'melhor-envio-3', 'melhor-envio-5']);
  });
});
